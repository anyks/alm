...
namespace anyks {
	...
	typedef class Word : public wstring {
		private:
			 ...
			 */
			static const Word trim(const wstring & str, const anyks::ocdc_t & meta, const locale & loc = locale::classic()) noexcept {
				...
				Word result(wstring{begin, rbegin.base()}, meta);
				// Выводим результат
				return result;
			}
		private:
